...
class MicEngine:
    def __init__(self):
        ...
        try:
            ...
            # MAX SENSITIVITY
            self.r.energy_threshold               = 15
            self.r.dynamic_energy_threshold       = False  # FIXED: keep sensitivity constant
    ...
    def _run(self):
        ...
        while self.running:
            try:
                ...
                small_ui = cv2.resize(ui, (960, 540))
                cv2.imshow("Pepper ASD Therapy Platform", small_ui)

                # Emotion window
                ew = self.em.get_window()
                if ew is not None:
                    ew_small = cv2.resize(ew, (320, 320))
                    cv2.imshow("Emotion (320x320)", ew_small)
